...
length show the resolved values. Read-only details stay focused on the effective
values without extra inheritance notes.
...
editing or copying a part's configuration. If an attached part is providing the
displayed value, a note identifies that source part by name and warns that
editing the firearm value will not change the displayed value while the part
remains attached.
